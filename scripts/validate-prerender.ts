...
function validatePrerenderHtml(id: string, slug: string): Issue[] {
  const issues: Issue[] = []
  ...
  // 6. (hreflang check removed — English-only site)
...
// 18. (ES/EN content parity check removed — English-only site)
...
function validateStructural(): Issue[] {
  ...
  for (const article of articleRegistry) {
    ...
    const htmlPath = resolve(dist, article.slug, 'index.html')
